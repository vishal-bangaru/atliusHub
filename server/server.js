...
app.post("/login",expressAsyncHandler(async(req,res)=>{
    ...
    if(isEqual==false)
     res.status(200).send({message:"Invalid password"})
        ...
        let token=jwt.sign({username:dbUser.username},"user",{expiresIn:60})
        delete dbUser.password
        res.status(201).send({message:user.username,jwtToken:token})
    }
   }

}))
app.get("/private",verifyToken,expressAsyncHandler(async(req,res)=>
    {
